...
use mock::{
	pBTC, pETH, pETHBTCPair, pUSDC, pUSDCBTCPair, pUSDCETHPair, Assets, DexModule, Event, ExtBuilder, ListingOrigin,
	Origin, Runtime, System, ALICE, BOB,
};
/*
use mock::{
...
fn list_provisioning_work() {
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_eq!(
			DexModule::trading_pair_statuses(pUSDCETHPair::get()),
		...
		System::assert_last_event(Event::DexModule(crate::Event::ListTradingPair(pUSDCETHPair::get())));
...
fn enable_diabled_trading_pair_work() {
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_noop!(
			DexModule::reenable_trading_pair(Origin::signed(ALICE), pUSDC, pETH),
			BadOrigin
		);
		...
		System::assert_last_event(Event::DexModule(crate::Event::EnableTradingPair(pUSDCETHPair::get())));

		assert_noop!(
			DexModule::reenable_trading_pair(Origin::signed(ListingOrigin::get()), pETH, pUSDC),
			Error::<Runtime>::MustBeNotEnabled
		);
	});
}
...
fn enable_provisioning_without_provision_work() {
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_eq!(
			DexModule::trading_pair_statuses(PUSDDOTPair::get()),
			TradingPairStatus::<_, _>::Provisioning(TradingPairProvisionParameters {
				min_contribution: (1_000_000_000_000u128, 1_000_000_000_000u128),
				target_provision: (5_000_000_000_000u128, 2_000_000_000_000u128),
				accumulated_provision: (0, 0),
		...
		System::assert_last_event(Event::DexModule(crate::Event::EnableTradingPair(PUSDDOTPair::get())));

		assert_noop!(
			DexModule::enable_trading_pair(Origin::signed(ListingOrigin::get()), PUSD, PBTC),
			Error::<Runtime>::MustBeNotEnabled
		);
	});
}
...
fn end_provisioning_trading_work() {
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_noop!(
			DexModule::end_provisioning(Origin::signed(ListingOrigin::get()), PUSD, PBTC),
			Error::<Runtime>::UnqualifiedProvision
		);
		System::set_block_number(10);

		assert_eq!(
			DexModule::trading_pair_statuses(PUSDBTCPair::get()),
			TradingPairStatus::<_, _>::Provisioning(TradingPairProvisionParameters {
				min_contribution: (1_000_000_000_000u128, 1_000_000_000_000u128),
				target_provision: (5_000_000_000_000u128, 2_000_000_000_000u128),
		...
		assert_eq!(
			DexModule::initial_share_exchange_rates(PUSDBTCPair::get()),
			Default::default()
		);
		assert_eq!(DexModule::liquidity_pool(PUSDBTCPair::get()), (0, 0));
		assert_eq!(Assets::total_issuance(PUSDBTCPair::get().dex_share_currency_id()), 0);
		assert_eq!(
			Assets::free_balance(PUSDBTCPair::get().dex_share_currency_id(), &DexModule::account_id()),
			0
		);
		...
		assert_eq!(
			DexModule::liquidity_pool(PUSDBTCPair::get()),
...
fn disable_trading_pair_work() {
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_noop!(
			DexModule::disable_trading_pair(Origin::signed(ALICE), PUSD, PDOT),
			BadOrigin
		);
		...
		System::assert_last_event(Event::DexModule(crate::Event::DisableTradingPair(PUSDDOTPair::get())));

		assert_noop!(
			DexModule::disable_trading_pair(Origin::signed(ListingOrigin::get()), PUSD, PDOT),
		...
		assert_noop!(
			DexModule::disable_trading_pair(Origin::signed(ListingOrigin::get()), PUSD, PBTC),
			Error::<Runtime>::MustBeEnabled
		);
	});
}
...
fn add_provision_work() {
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_eq!(DexModule::provisioning_pool(PUSDDOTPair::get(), ALICE), (0, 0));
		...
		assert_eq!(Assets::free_balance(PUSD, &DexModule::account_id()), 0);
		assert_eq!(Assets::free_balance(PDOT, &DexModule::account_id()), 0);
		...
		assert_eq!(
			DexModule::provisioning_pool(PUSDDOTPair::get(), ALICE),
			(5_000_000_000_000u128, 0)
		);
		assert_eq!(Assets::free_balance(PUSD, &ALICE), 999_995_000_000_000_000u128);
		assert_eq!(Assets::free_balance(PDOT, &ALICE), 1_000_000_000_000_000_000u128);
		assert_eq!(
			Assets::free_balance(PUSD, &DexModule::account_id()),
			5_000_000_000_000u128
		);
		assert_eq!(Assets::free_balance(PDOT, &DexModule::account_id()), 0);
...
fn claim_dex_share_work() {
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_noop!(
			DexModule::claim_dex_share(Origin::signed(ALICE), ALICE, PUSD, PDOT),
		...
		assert_eq!(
			InitialShareExchangeRates::<Runtime>::contains_key(PUSDDOTPair::get()),
			true
		);
		assert_eq!(
			DexModule::initial_share_exchange_rates(PUSDDOTPair::get()),
			(ExchangeRate::one(), ExchangeRate::saturating_from_rational(5, 1))
		);
		assert_eq!(
			Assets::free_balance(lp_currency_id, &DexModule::account_id()),
		...
		assert_eq!(
			DexModule::provisioning_pool(PUSDDOTPair::get(), BOB),
		...
		assert_ok!(DexModule::claim_dex_share(Origin::signed(ALICE), ALICE, PUSD, PDOT));
		assert_eq!(
			Assets::free_balance(lp_currency_id, &DexModule::account_id()),
		...
		assert_ok!(DexModule::claim_dex_share(Origin::signed(BOB), BOB, PUSD, PDOT));
		assert_eq!(Assets::free_balance(lp_currency_id, &DexModule::account_id()), 0);
		assert_eq!(DexModule::provisioning_pool(PUSDDOTPair::get(), BOB), (0, 0));
...
fn get_liquidity_work() {
	ExtBuilder::default().build().execute_with(|| {
		LiquidityPool::<Runtime>::insert(PUSDDOTPair::get(), (1000, 20));
		assert_eq!(DexModule::liquidity_pool(PUSDDOTPair::get()), (1000, 20));
		assert_eq!(DexModule::get_liquidity(PUSD, PDOT), (1000, 20));
		assert_eq!(DexModule::get_liquidity(PDOT, PUSD), (20, 1000));
	});
}
...
fn get_target_amounts_work() {
	ExtBuilder::default()
		...
		.build()
		.execute_with(|| {
			LiquidityPool::<Runtime>::insert(PUSDDOTPair::get(), (50000, 10000));
			LiquidityPool::<Runtime>::insert(PUSDBTCPair::get(), (100000, 10));
			assert_noop!(
				DexModule::get_target_amounts(&vec![PDOT], 10000, None),
				Error::<Runtime>::InvalidTradingPathLength,
			);
			assert_noop!(
				DexModule::get_target_amounts(&vec![PDOT, PUSD, PBTC, PDOT], 10000, None),
				Error::<Runtime>::InvalidTradingPathLength,
			);
			assert_noop!(
				DexModule::get_target_amounts(&vec![PDOT, PUSD, ACA], 10000, None),
				Error::<Runtime>::MustBeEnabled,
			);
			assert_eq!(
				DexModule::get_target_amounts(&vec![PDOT, PUSD], 10000, None),
				Ok(vec![10000, 24874])
			);
			assert_eq!(
				DexModule::get_target_amounts(&vec![PDOT, PUSD], 10000, Ratio::checked_from_rational(50, 100)),
				Ok(vec![10000, 24874])
			);
			assert_noop!(
				DexModule::get_target_amounts(&vec![PDOT, PUSD], 10000, Ratio::checked_from_rational(49, 100)),
				Error::<Runtime>::ExceedPriceImpactLimit,
			);
			assert_eq!(
				DexModule::get_target_amounts(&vec![PDOT, PUSD, PBTC], 10000, None),
				Ok(vec![10000, 24874, 1])
			);
			assert_noop!(
				DexModule::get_target_amounts(&vec![PDOT, PUSD, PBTC], 100, None),
				Error::<Runtime>::ZeroTargetAmount,
			);
			assert_noop!(
				DexModule::get_target_amounts(&vec![PDOT, PBTC], 100, None),
...
fn get_supply_amounts_work() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			LiquidityPool::<Runtime>::insert(PUSDBTCPair::get(), (100000, 10));
			assert_noop!(
				DexModule::get_supply_amounts(&vec![PDOT], 10000, None),
				Error::<Runtime>::InvalidTradingPathLength,
			);
			assert_noop!(
				DexModule::get_supply_amounts(&vec![PDOT, PUSD, PBTC, PDOT], 10000, None),
				Error::<Runtime>::InvalidTradingPathLength,
			);
			assert_noop!(
				DexModule::get_supply_amounts(&vec![PDOT, PUSD, ACA], 10000, None),
				Error::<Runtime>::MustBeEnabled,
			);
			assert_eq!(
				DexModule::get_supply_amounts(&vec![PDOT, PUSD], 24874, None),
				Ok(vec![10000, 24874])
			);
			assert_eq!(
				DexModule::get_supply_amounts(&vec![PDOT, PUSD], 25000, Ratio::checked_from_rational(50, 100)),
				Ok(vec![10102, 25000])
			);
			assert_noop!(
				DexModule::get_supply_amounts(&vec![PDOT, PUSD], 25000, Ratio::checked_from_rational(49, 100)),
				Error::<Runtime>::ExceedPriceImpactLimit,
			);
			assert_noop!(
				DexModule::get_supply_amounts(&vec![PDOT, PUSD, PBTC], 10000, None),
				Error::<Runtime>::ZeroSupplyAmount,
			);
			assert_noop!(
				DexModule::get_supply_amounts(&vec![PDOT, PBTC], 10000, None),
				Error::<Runtime>::InsufficientLiquidity,
			);
		});
}
...
fn _swap_work() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_eq!(DexModule::get_liquidity(PUSD, PDOT), (50000, 10000));
			assert_noop!(
				DexModule::_swap(PUSD, PDOT, 50000, 5001),
				Error::<Runtime>::InvariantCheckFailed
			);
			assert_ok!(DexModule::_swap(PUSD, PDOT, 50000, 5000));
			assert_eq!(DexModule::get_liquidity(PUSD, PDOT), (100000, 5000));
			assert_ok!(DexModule::_swap(PDOT, PUSD, 100, 800));
			assert_eq!(DexModule::get_liquidity(PUSD, PDOT), (99200, 5100));
		});
}
...
fn _swap_by_path_work() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_eq!(DexModule::get_liquidity(PUSD, PDOT), (50000, 10000));
			assert_eq!(DexModule::get_liquidity(PUSD, PBTC), (100000, 10));
			assert_ok!(DexModule::_swap_by_path(&vec![PDOT, PUSD], &vec![10000, 25000]));
			assert_eq!(DexModule::get_liquidity(PUSD, PDOT), (25000, 20000));
			assert_ok!(DexModule::_swap_by_path(&vec![PDOT, PUSD, PBTC], &vec![100000, 20000, 1]));
			assert_eq!(DexModule::get_liquidity(PUSD, PDOT), (5000, 120000));
			assert_eq!(DexModule::get_liquidity(PUSD, PBTC), (120000, 9));
		});
}
...
fn add_liquidity_work() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_noop!(
				DexModule::add_liquidity(Origin::signed(ALICE), ACA, PUSD, 100_000_000, 100_000_000, 0, false),
				Error::<Runtime>::MustBeEnabled
			);
			assert_noop!(
				DexModule::add_liquidity(Origin::signed(ALICE), PUSD, PDOT, 0, 100_000_000, 0, false),
				Error::<Runtime>::InvalidLiquidityIncrement
			);

			assert_eq!(DexModule::get_liquidity(PUSD, PDOT), (0, 0));
			assert_eq!(Assets::free_balance(PUSD, &DexModule::account_id()), 0);
			assert_eq!(Assets::free_balance(PDOT, &DexModule::account_id()), 0);
			assert_eq!(
			...
			assert_eq!(
				DexModule::get_liquidity(PUSD, PDOT),
				(5_000_000_000_000, 1_000_000_000_000)
			);
			assert_eq!(Assets::free_balance(PUSD, &DexModule::account_id()), 5_000_000_000_000);
			assert_eq!(Assets::free_balance(PDOT, &DexModule::account_id()), 1_000_000_000_000);
			assert_eq!(
				Assets::free_balance(PUSDDOTPair::get().dex_share_currency_id(), &ALICE),
				10_000_000_000_000
			);
			...
			assert_noop!(
				DexModule::add_liquidity(Origin::signed(BOB), PUSD, PDOT, 4, 1, 0, true,),
				Error::<Runtime>::InvalidLiquidityIncrement,
			);
			...
			assert_eq!(Assets::free_balance(PUSD, &DexModule::account_id()), 45_000_000_000_000);
			assert_eq!(Assets::free_balance(PDOT, &DexModule::account_id()), 9_000_000_000_000);
			assert_eq!(
				Assets::free_balance(PUSDDOTPair::get().dex_share_currency_id(), &BOB),
				0
			);
...
fn remove_liquidity_work() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_eq!(Assets::free_balance(PUSD, &DexModule::account_id()), 1_000_000_000_000);
			assert_eq!(Assets::free_balance(PDOT, &DexModule::account_id()), 200_000_000_000);
			assert_eq!(
				Assets::free_balance(PUSDDOTPair::get().dex_share_currency_id(), &ALICE),
				2_000_000_000_000
			);
...
fn do_swap_with_exact_supply_work() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_eq!(
				DexModule::get_liquidity(PUSD, PBTC),
			...
			assert_eq!(Assets::free_balance(PDOT, &DexModule::account_id()), 100_000_000_000_000);
			assert_eq!(Assets::free_balance(PBTC, &DexModule::account_id()), 10_000_000_000);
			assert_eq!(Assets::free_balance(PUSD, &BOB), 1_000_000_000_000_000_000);
			assert_eq!(Assets::free_balance(PDOT, &BOB), 1_000_000_000_000_000_000);
			...
			assert_noop!(
				DexModule::do_swap_with_exact_supply(&BOB, &[PDOT, PUSD, PBTC, PDOT], 100_000_000_000_000, 0, None),
				Error::<Runtime>::InvalidTradingPathLength,
			);
			assert_noop!(
				DexModule::do_swap_with_exact_supply(&BOB, &[PDOT, ACA], 100_000_000_000_000, 0, None),
				Error::<Runtime>::MustBeEnabled,
			);
			...
			assert_eq!(Assets::free_balance(PDOT, &DexModule::account_id()), 200_000_000_000_000);
			assert_eq!(Assets::free_balance(PBTC, &DexModule::account_id()), 10_000_000_000);
			...
			assert_eq!(Assets::free_balance(PDOT, &DexModule::account_id()), 400_000_000_000_000);
			assert_eq!(Assets::free_balance(PBTC, &DexModule::account_id()), 4_469_336_163);
			assert_eq!(Assets::free_balance(PUSD, &BOB), 1_000_248_743_718_592_964);
...
fn do_swap_with_exact_target_work() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_noop!(
				DexModule::do_swap_with_exact_target(&BOB, &[PDOT, ACA], 250_000_000_000_000, 200_000_000_000_000, None),
				Error::<Runtime>::MustBeEnabled,
			);
			...
			assert_eq!(Assets::free_balance(PDOT, &DexModule::account_id()), 201_010_101_010_102);
			assert_eq!(Assets::free_balance(PBTC, &DexModule::account_id()), 10_000_000_000);
			...
			assert_eq!(Assets::free_balance(PDOT, &DexModule::account_id()), 338_664_681_397_095);
			assert_eq!(Assets::free_balance(PBTC, &DexModule::account_id()), 5_000_000_000);
			assert_eq!(Assets::free_balance(PUSD, &BOB), 1_000_250_000_000_000_000);
...
fn initialize_added_liquidity_pools_genesis_work() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_eq!(DexModule::get_liquidity(PUSD, PDOT), (1000000, 2000000));
			assert_eq!(Assets::free_balance(PUSD, &DexModule::account_id()), 2000000);
			assert_eq!(Assets::free_balance(PDOT, &DexModule::account_id()), 3000000);
			assert_eq!(
				Assets::free_balance(PUSDDOTPair::get().dex_share_currency_id(), &ALICE),
				2000000
			);
