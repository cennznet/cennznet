...
use super::{
	BalanceOf, Config, EraIndex, Exposure, NegativeImbalanceOf, Pallet, Perbill, SessionInterface, Store,
	UnappliedSlash,
};
...
pub(crate) fn compute_slash<T: Config>(params: SlashParams<T>) -> Option<UnappliedSlash<T::AccountId, BalanceOf<T>>> {
	...
	let (prior_slash_p, _era_slash) =
		<Pallet<T> as Store>::ValidatorSlashInEra::get(&slash_era, stash).unwrap_or((Perbill::zero(), Zero::zero()));
...
fn slash_nominators<T: Config>(
	...
	for nominator in &exposure.others {
		...
		let era_slash = {
			...
			let mut era_slash =
				<Pallet<T> as Store>::NominatorSlashInEra::get(&slash_era, stash).unwrap_or(Zero::zero());
...
fn do_slash<T: Config>(
	...
	if !value.is_zero() {
		...
		// trigger the event
		<Pallet<T>>::deposit_event(super::RawEvent::Slash(stash.clone(), value));
	}
}
