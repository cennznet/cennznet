...
use frame_support::{
	...
	traits::{Currency, Get, Imbalance, LockIdentifier, LockableCurrency, OnUnbalanced, UnixTime, WithdrawReasons},
...
use sp_runtime::{
	...
	traits::{AtLeast32Bit, CheckedDiv, CheckedSub, Convert, SaturatedConversion, Saturating, StaticLookup, Zero},
...
pub type RewardBalanceOf<T> =
	<<T as Trait>::RewardCurrency as Currency<<T as frame_system::Trait>::AccountId>>::Balance;
...
decl_storage! {
	trait Store for Module<T: Trait> as Staking {
		...
		pub ErasValidatorReward get(fn eras_validator_reward):
			map hasher(twox_64_concat) EraIndex => Option<RewardBalanceOf<T>>;
		...
		/// Transaction payment rewards for elected validators
		pub ErasFeeReward get(fn eras_fee_reward) :
			map hasher(twox_64_concat) EraIndex => Option<RewardBalanceOf<T>>;
...
decl_event!(
	pub enum Event<T> where Balance = BalanceOf<T>, <T as frame_system::Trait>::AccountId, RewardBalance = RewardBalanceOf<T> {
		...
		Reward(AccountId, RewardBalance),
...
impl<T: Trait> Module<T> {
		...
		for (validator, nominator_index) in validators.into_iter() {
			...
			if let Some(nominator_exposure) = validator_exposure.others.get(nominator_index as usize) {
				...
				let nominator_exposure_part =
					Perbill::from_rational_approximation(nominator_exposure.value, validator_exposure.total);
	...
	fn do_payout_validator(who: T::AccountId, era: EraIndex) -> DispatchResult {
		...
		let num_of_fee_recipients = Self::elected_count(era) as u32;
		// Transaction fee equally distributed among all elected validators as reward
		let fee_reward = <ErasFeeReward<T>>::get(era)
			.unwrap_or(Zero::zero())
			.checked_div(&<RewardBalanceOf<T>>::from(num_of_fee_recipients))
			.unwrap_or(Zero::zero());
		if !fee_reward.is_zero() {
			Self::deposit_event(RawEvent::RewardFees(fee_reward, num_of_fee_recipients));
		}

		if let Some(imbalance) = Self::make_payout(&ledger.stash, fee_reward + reward * era_payout) {
			Self::deposit_event(RawEvent::Reward(who, imbalance.peek()));
		}
	...
	fn end_era(active_era: ActiveEraInfo, _session_index: SessionIndex) {
		...
		if let Some(active_era_start) = active_era.start {
			...
			let total_rewarded_stake =
				RewardBalanceOf::<T>::saturated_from(Self::eras_total_stake(&active_era.index).saturated_into()); // ugly hack to get `T::RewardCurrency` balance from `T::Currency` balance
			let (total_payout, _max_payout) = inflation::compute_total_payout::<RewardBalanceOf<T>>(
				&T::RewardCurve::get(),
	...
	fn clear_era_information(era_index: EraIndex) {
		...
		<ErasFeeReward<T>>::remove(era_index);
	}

	pub fn add_to_current_era_transaction_fee_reward(amount: RewardBalanceOf<T>) {
		if let Some(active_era) = Self::active_era() {
			ErasFeeReward::<T>::mutate(active_era.index, |era_fee_reward| {
				let current_reward = era_fee_reward.unwrap_or(Zero::zero());
				*era_fee_reward = Some(current_reward.saturating_add(amount));
			});
		}
	}
	...
	pub fn elected_count(index: EraIndex) -> usize {
		let exposed_stakers = <ErasStakers<T> as IterableStorageDoubleMap<
			EraIndex,
			T::AccountId,
			Exposure<T::AccountId, BalanceOf<T>>,
		>>::iter(index)
		.filter(|x| x.1.total > Zero::zero());
		exposed_stakers.count()
	}
	...
	fn on_offence(
		...
		for (details, slash_fraction) in offenders.iter().zip(slash_fraction) {
			...
			if Self::invulnerables().contains(stash) {
				...
				frame_support::print("Invulnerable validator not slashed");
