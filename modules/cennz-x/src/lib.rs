...
impl<T: Trait> Module<T> {
    ...
    pub fn make_asset_to_core_swap_output(
        ...
        } else if <generic_asset::Module<T>>::free_balance(&core_asset_id, &exchange_address)
            < amount_bought
        {
    ...
    pub fn get_asset_to_core_output_price(
        ...
        if amount_bought > Zero::zero() {
            ...
            let exchange_key = (core_asset_id, *asset_id);
            let exchange_address = Self::generate_exchange_address(&exchange_key);
            let trade_asset_reserve =
                <generic_asset::Module<T>>::free_balance(asset_id, &exchange_address);
            let core_asset_reserve =
                <generic_asset::Module<T>>::free_balance(&core_asset_id, &exchange_address);
...
mod tests {
    ...
    fn the_first_investor_can_add_liquidity() {
        with_externalities(&mut ExtBuilder::default().build(), || {
            ...
            assert_eq!(
                <generic_asset::Module<Test>>::free_balance(&0, &pool_address),
                10
            );
            assert_eq!(
                <generic_asset::Module<Test>>::free_balance(&1, &pool_address),
                15
            );
    ...
    fn get_asset_to_core_output_price() {
        with_externalities(&mut ExtBuilder::default().build(), || {
            ...
            assert_eq!(
                CennzXSpot::get_asset_to_core_output_price(&1, 123, fee_rate),
                140
            );
    ...
    fn get_asset_to_core_output_price_internal_call() {
        with_externalities(&mut ExtBuilder::default().build(), || {
            ...
            assert_eq!(
                CennzXSpot::get_asset_to_core_output_price(&1, 5, fee_rate),
                1003
            );
            assert_eq!(
