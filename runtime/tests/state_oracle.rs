...
fn callback_execution() {
	ExtBuilder::default().build().execute_with(|| {
		...
		// Test
		let result = StateOracleCallbackExecutor::<Runtime>::execute(
			&caller,
		...
		assert_eq!(System::events().len(), 1_usize);
		if let EventRecord { 
            event: Event::Ethereum(pallet_ethereum::Event::Executed(caller, _, _, _)),
            phase: _,
            topics: _,
        } = System::events()[0] {
			// ethereum pallet current receipts has the callback tx
