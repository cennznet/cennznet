	...
	B: Block,
{
	fn validate(
		...
		if let Ok(msg) = VoteMessage::<MmrRootHash, NumberFor<B>, Public, Signature>::decode(&mut data) {
			if EthyKeystore::verify(&msg.id, &msg.signature, &msg.commitment.encode()) {
	...
	fn message_expired<'a>(&'a self) -> Box<dyn FnMut(B::Hash, &[u8]) -> bool + 'a> {
		...
		Box::new(move |_topic, mut data| {
			...
			trace!(target: "ethy", "💎 Message for round #{} expired: {}", msg.commitment.block_number, expired);
	...
	fn message_allowed<'a>(&'a self) -> Box<dyn FnMut(&PeerId, MessageIntent, &B::Hash, &[u8]) -> bool + 'a> {
		...
		Box::new(move |_who, _intent, _topic, mut data| {
			...
			trace!(target: "ethy", "💎 Message for round #{} allowed: {}", msg.commitment.block_number, allowed);
