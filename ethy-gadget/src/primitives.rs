...
//! Primitives for ETHY protocol.
//!
//! The crate contains shared data types used by ETHY protocol and documentation (in a form of
//! code) for building a ETHY light client.
//!
//! ETHY is a gadget that runs alongside another finality gadget (for instance GRANDPA).
//! For simplicity (and the initially intended use case) the documentation says GRANDPA in places
//! where a more abstract "Finality Gadget" term could be used, but there is no reason why ETHY
//! wouldn't run with some other finality scheme.
//! ETHY validator set is supposed to be tracking the Finality Gadget validator set, but note that
//! it will use a different set of keys. For Polkadot use case we plan to use `secp256k1` for ETHY,
...
/// Key type for ETHY module.
pub const KEY_TYPE: sp_application_crypto::KeyTypeId = sp_application_crypto::KeyTypeId(*b"ethy");

/// ETHY cryptographic types
///
...
pub mod crypto {
	...
	/// Identity of a ETHY authority using ECDSA as its crypto.
	pub type AuthorityId = Public;

	/// Signature for a ETHY authority using ECDSA as its crypto.
